...
- [x] **T5.10  Calls UI** *(web UI done + verified live; real media + cross-device incoming ring = LiveKit/WS seams; group grid deferred; mobile with T5.01)*
  DONE (web): the 1:1 call UI already existed (CallOverlay: outgoing/incoming ring accept/decline, connecting/connected, ended, controls camera/flip/screen-share/blur + End, local self-preview) wired to the REST control plane (`/v1/calls` create/answer/decline/rejoin) + the @wa/call-engine state machine + SFrame E2EE. Added the missing **call-history screen** (`CallHistory` over `GET /v1/calls/history`, 📞 Calls entry in the chat-list header, `AppServices.callHistory`). VERIFIED LIVE: 📞 in a thread → createCall → overlay shows **"Ringing…"** with an End control (control plane + state machine working); the **Calls screen** renders ("No calls yet." — history endpoint reached, empty in dev). **SEAMS (unchanged from Phase 2, no LiveKit in the dev stack):** real audio/video rides `stubRtc` until the LiveKit SDK is wired; cross-device **incoming ring** needs the WS `dev.{id}.call` frame decode (the transport still drops call frames); **group grid ≤32** deferred. web typecheck green. Mobile parity rides T5.01.
